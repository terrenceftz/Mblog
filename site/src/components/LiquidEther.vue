...
// Liquid Ether 背景动效：移植自 reactbits.dev (DavidHDev/react-bits, MIT + Commons Clause) — 个人博客非商用使用
import { onBeforeUnmount, onMounted, ref, watch } from 'vue';
import * as THREE from 'three';

// ============================================================
// Props（默认值与原版 React 组件保持一致）
// ============================================================
...
// ============================================================
// 调色板纹理（每次构建模拟时按 colors 重建）
// ============================================================
let paletteTex: THREE.DataTexture | null = null;
...
class AutoDriver {
  ...
  activationTime = 0;
  margin = 0.2;
  _tmpDir = new THREE.Vector2(); // reuse temp vector to avoid per-frame alloc
...
class WebGLManager {
  ...
  applyOptions(o: any) {
    ...
    if (o.resolution !== prevRes) {
      sim.resize();
    }
  }

  updateAutoDriver(o: any) {
    if (this.autoDriver) {
      this.autoDriver.enabled = o.autoDemo;
      this.autoDriver.speed = o.autoSpeed;
      this.autoDriver.resumeDelay = o.autoResumeDelay;
      this.autoDriver.rampDurationMs = o.autoRampDuration * 1000;
      if (this.autoDriver.mouse) {
        this.autoDriver.mouse.autoIntensity = o.autoIntensity;
        this.autoDriver.mouse.takeoverDuration = o.takeoverDuration;
      }
    }
  }
...
// 生命周期：构建 / 启动 / 停止模拟
// ============================================================
function isNormalTheme(): boolean {
...
function buildSim(): WebGLManager | null {
  const el = canvasRef.value;
  if (!el) return null;
  paletteTex = makePaletteTexture(props.colors);
  const container = el as HTMLCanvasElement;
  container.style.position = container.style.position || 'relative';
  container.style.overflow = container.style.overflow || 'hidden';

  const mgr = new WebGLManager({
    $wrapper: container,
  ...
  mgr.applyOptions(props as any);
  mgr.start();
  return mgr;
}

function tearDown(mgr: WebGLManager | null) {
  if (rafId) {
    cancelAnimationFrame(rafId);
    rafId = null;
  }
...
// Vue 生命周期绑定
// ============================================================
let themeHandler: EventListener | null = null;
...
onMounted(() => {
  ...
  window.addEventListener('mblog-theme-change', themeHandler);
});

onBeforeUnmount(() => {
  if (themeHandler) {
    window.removeEventListener('mblog-theme-change', themeHandler);
    themeHandler = null;
  }
  stop();
});

// 模拟参数变化 → 热更新 sim.options（对应原版第二个 useEffect）
watch(
  () => [
    props.mouseForce,
    props.cursorSize,
  ...
  () => {
    if (webgl) webgl.applyOptions(props as any);
  }
);

// 调色板变化 → 整体重建（原版 colors 位于主 effect 依赖中，等价于重建）
watch(
  () => props.colors,
  () => {
    if (!simActive) return;
...
watch(
  ...
  () => {
    if (webgl) webgl.updateAutoDriver(props as any);
  }
);
</script>
